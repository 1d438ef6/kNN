...
public class Netz {
     ...
     * @return ArrayList<Integer>
     */
    public ArrayList<Integer> getUseablePointClasses() {
        ArrayList<Integer> ret = new ArrayList<>();
        boolean add = true;
        for (int i = 0; i < this.dp.size(); i++) {
            for (int j = 0; j < ret.size(); j++) {
                if (dp.get(i).getPointClass() == ret.get(j) || !dp.get(i).isUseable()) {
                    add = false;
                }
            }
            if (add) {
                ...
                for (int j = 0; j < this.dp.size(); j++) {
                    ...
                    if (this.dp.get(j).getPointClass() == 0) {
                        ...
                        for (int i2 = 0; i2 < nn.size(); i2++) {
                            //Distanzen berechnen
                            dist.add(this.dp.get(j).dist(nn.get(i2), funktion));
                        }
                        for (int i2 = 0; i2 < nn.size(); i2++) {
                            //Gewichtsformel
    ...
    public int classifyPoint(DataPoint dp, int k, boolean weighted, String funktion) {
        if (weighted) {
            ...
            //Liste mit den k nächsten Nachbarn
            ArrayList<DataPoint> nn = this.getNextUseableNeighbours(dp, k);
            //Liste für die distanzen der Punkte
            ArrayList<Double> dist = new ArrayList<>();
            //array für die gewichteten Distanzen
            //sortiert nach Klassen
            ...
            for (int i2 = 0; i2 < nn.size(); i2++) {
                //Distanzen berechnen
                dist.add(dp.dist(nn.get(i2), funktion));
            }
            for (int i2 = 0; i2 < nn.size(); i2++) {
                //Gewichtsformel
                //if dk != d1 : (dk-di)/(dk-d1)
            ...
            ArrayList<DataPoint> nn = this.getNextUseableNeighbours(dp, k, funktion);
            Counter c = this.getNPointClasses(nn);
            c.sort();
            return c.getX().get(0);
        }
